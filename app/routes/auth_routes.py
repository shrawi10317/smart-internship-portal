    ...
    if request.method == "POST":
        if "email" in request.form:
            ...
            if user:
                ...
                msg = Message(subject="Your OTP Code", sender=current_app.config.get("MAIL_DEFAULT_SENDER"), recipients=[email], body=f"Your OTP is: {otp}")
                threading.Thread(target=send_async_email, args=(current_app._get_current_object(), msg)).start()
...
def resend_otp():
    ...
    otp = str(random.randint(100000, 999999))
    session["otp"] = otp
    msg = Message(subject="Your OTP - SmartInternship", sender=current_app.config.get("MAIL_DEFAULT_SENDER"), recipients=[user.email], body=f"Hello {user.name}, your new OTP is: {otp}")
    threading.Thread(target=send_async_email, args=(current_app._get_current_object(), msg)).start()
